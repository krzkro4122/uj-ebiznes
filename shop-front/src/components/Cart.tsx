      ...
      <ul className="pl-4">
        {items.map((item) => (
          <li key={item.id} className='shadow-sm flex justify-between pt-2'>
            <span className="">
              <span className=" pr-2">
                <button className="text-4xl m-0.5 mr-1 pb-1.5 pr-2 pl-2 rounded-md border shadow-md border-gray-400 bg-gray-500 opacity-80 text-gray-100 hover:bg-gray-100 hover:text-gray-500 active:bg-gray-300 "
                  onClick={() => item.quantity! < item.stock ? updateItemQuantity(item.id, item.quantity! + 1) : alert("None left!")}
                  >
                  +
                </button>
                <button className="m-0.5 pr-3 pb-1.5 pl-3 text-4xl rounded-md shadow-md border border-gray-300 bg-gray-100 text-gray-500 opacity-80 hover:bg-gray-500 hover:text-gray-100 active:bg-gray-300 "
